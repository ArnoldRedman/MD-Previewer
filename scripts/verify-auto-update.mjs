...
let desktopScript = mainRs
  ...
  .replaceAll('{{', '{')
  .replaceAll('}}', '}')
  .replaceAll('{cargo_version}', '1.4.0')
...
const versionComparisonResult = await page.evaluate(() => {
  ...
  return {
    test1: isNewer('v1.4.1', '1.4.0'),     // true
    test2: isNewer('v1.5.0', '1.4.0'),     // true
    test3: isNewer('v1.4.0', '1.4.0'),     // false
    test4: isNewer('v1.3.1', '1.4.0'),     // false
    test5: isNewer('v1.10.0', '1.4.0'),    // true
    test6: isNewer('1.4.1', '1.4.0'),      // true
  };
});
...
await page.evaluate(() => {
  ...
  const fakeRelease = {
    tag_name: 'v1.4.1',
    name: 'v1.4.1 — Test Update',
    body: '## 1.4.1 Notes\n- Fixed **something**',
    html_url: 'https://github.com/ArnoldRedman/MD-Previewer/releases/tag/v1.4.1',
    assets: [
      { name: 'MD-Previewer-Setup.exe', browser_download_url: 'https://github.com/ArnoldRedman/MD-Previewer/releases/download/v1.4.1/MD-Previewer-Setup.exe' },
      { name: 'MD-Previewer-windows-x64.exe', browser_download_url: 'https://github.com/ArnoldRedman/MD-Previewer/releases/download/v1.4.1/MD-Previewer-windows-x64.exe' }
    ]
  };
...
if (badgeText !== 'v1.4.1') throw new Error(`Expected badge text 'v1.4.1', got '${badgeText}'`);

const releaseName = await page.$eval('#update-release-name', el => el.textContent.trim());
if (releaseName !== 'v1.4.1 — Test Update') throw new Error(`Expected release name 'v1.4.1 — Test Update', got '${releaseName}'`);
...
if (notesRequest !== 'render-release-notes:## 1.4.1 Notes\n- Fixed **something**') {
...
await page.evaluate(() => {
  window.__setUpdateNotes('<h2 id="141-notes">1.4.1 Notes</h2>\n<ul>\n<li>Fixed <strong>something</strong></li>\n</ul>\n');
});
...
if (renderedNotes.heading !== '1.4.1 Notes' || renderedNotes.strong !== 'something') {
...
const hasSelfUpdateMsg = messages.some(m => m.startsWith('self-update:https://github.com/ArnoldRedman/MD-Previewer/releases/download/v1.4.1/MD-Previewer-windows-x64.exe'));
if (!hasSelfUpdateMsg) {
...
if (!checkBtnBox.hasUpdate || !checkBtnBox.text.includes('v1.4.1')) {
