...
final class PreviewViewController: UIViewController {
    ...
    private lazy var webView: WKWebView = {
        ...
        configuration.userContentController.add(self, name: "mdPreviewer")
    ...
    private func renderDocument(at url: URL, displayName preferredName: String? = nil, shouldSaveRecent: Bool = true) {
        ...
        do {
            ...
            webView.evaluateJavaScript("window.MDPreviewer && window.MDPreviewer.render(\(json));")
        } catch {
            let message = "Cannot read \(documentName)"
            webView.evaluateJavaScript("window.MDPreviewer && window.MDPreviewer.render({markdown:\(message.jsStringLiteral),name:'Read error.md',baseHref:''});")
        }
    }
    ...
    private func printDocument() {
        ...
        info.jobName = webView.title?.replacingOccurrences(of: " - MD Previewer", with: "") ?? "MD Previewer"
    ...
    private func sendRecentToWeb() {
        ...
        webView.evaluateJavaScript("window.MDPreviewer && window.MDPreviewer.setRecent(\(json));")
    }
...
extension PreviewViewController: WKScriptMessageHandler {
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        guard message.name == "mdPreviewer" else {
            return
        }
